...
class ConfigView(tk.Frame):
    def __init__(self, parent, controller):
        ...
        self.region_combo = ttk.Combobox(self, state="readonly", values=[region.name for region in HarveyRegion],
                                         width=10)
        self.region_combo.bind("<<ComboboxSelected>>",
                               lambda _: self.controller.api_region.set(self.region_combo.get()))
    ...
    def handle_continue(self):
        if self.controller.api_region.get() not in [region.name for region in
                                                    HarveyRegion] or self.api_key_entry.get().strip() == "":
